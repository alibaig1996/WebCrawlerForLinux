...
bool session::check_html(){
    ...
    std::ifstream source("/home/baigoo365/Proj_2/source.txt");
    std::string s = "";
    do{
...
void urls_parsed(std::string x){
    std::ofstream urls("/home/baigoo365/Proj_2/urls.txt", std::ios::app|std::ios::out);
...
bool check_urls(std::string x){
    std::ifstream urls("/home/baigoo365/Proj_2/urls.txt", std::ios::in);
...
void session::stringsearch(std::string word){
    ...
    do{
        ...
        std::ofstream FILE("/home/baigoo365/Proj_2/source.txt");

        std::ofstream wordfile("/home/baigoo365/Proj_2/index.txt", std::ios::app|std::ios::out);
        ...
        if (check_html() == false){
            std::cout << "Webpage not in html format. Skipping... " << std::endl << std::endl;
            list.pop();
            continue;
        }
        ...
        if (count > 1 && check_urls(list.front()) == true){
            std::cout << "Webpage has already been crawled. Skipping...\n\n";
            list.pop();
            continue;
        }

        // Opens file for reading
        std::ifstream file("/home/baigoo365/Proj_2/source.txt");
        ...
        int counter; // Counter variable declared to count number of lines

        // Condition check to ensure file is open
        if (file.is_open()){

           counter = 1; // Value initialised to 1
           ...
           do{
                ...
                if (wordfile.is_open()){
                    ...
                    std::size_t point = s1.find(target);

                    if (point != std::string::npos){

                        wordfile << list.front() << "\tLine number: " << counter << std::endl;
                        wordfile << target << std::endl;
                    }
                }
                else{
                ...
                counter++; // Increment at end of one iteration
    ...
    // Check to see if wordfile is empty
       int length;
       std::ifstream wordfile;

       wordfile.open("/home/baigoo365/Proj_2/index.txt", std::ios::binary); // Open the file
       wordfile.seekg(0, std::ios::end); // Put the "cursor" at the end of the file
       length = wordfile.tellg(); // Find the position of the cursor
       wordfile.close(); // Close the file

       if ( length == 0 ){
           std::ofstream wordfile("/home/baigoo365/Proj_2/index.txt", std::ios::app);
           wordfile << "No matched found!";
           wordfile.close();
       }

    remove("/home/baigoo365/Proj_2/source.txt");
    remove("/home/baigoo365/Proj_2/urls.txt");
